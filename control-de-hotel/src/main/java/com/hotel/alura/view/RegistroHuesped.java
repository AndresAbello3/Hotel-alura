...
public class RegistroHuesped extends JFrame {
	...
	public RegistroHuesped(int idReserva) {
		...
		//configurarAccionesDelFormularioDesdeRegistroHuesped();
		...
		txtNumeroReserva.setBounds(560, 495, 285, 33);
		txtNumeroReserva.setColumns(10);
		txtNumeroReserva.setBackground(Color.WHITE);
		txtNumeroReserva.setBorder(javax.swing.BorderFactory.createEmptyBorder());
		
		// ESTABLECE EL VALOR DEL CAMPO txtNReserva Y DESACTIVA LA EDICIÓN.
	    txtNumeroReserva.setText(String.valueOf(idReserva));
...
///////////////////////////////////////////////////
	...
	private void guardar() {
        if (txtFechaNacimiento.getDate() != null && !txtNombre.getText().isEmpty() && !txtApellido.getText().isEmpty()
                && !txtTelefono.getText().isEmpty()) {
            LocalDate fechaNacimiento = LocalDate.parse(((JTextField) txtFechaNacimiento.getDateEditor().getUiComponent())
                    .getText());
            int idReserva = Integer.parseInt(txtNumeroReserva.getText());

            Huesped huespedes = new Huesped(txtNombre.getText(), txtApellido.getText(), fechaNacimiento,
                    txtNacionalidad.getSelectedItem().toString(), txtTelefono.getText(), idReserva);
            this.huespedController.guardar(huespedes);

            // Llama al método para mostrar el "check verde" y "Dato guardado"
            check = mostrarCheckVerde();

            // Usar Timer para esperar 3 segundos antes de mostrar la pregunta
    ...
    private JLabel mostrarCheckVerde() {
        // Mostrar "check verde" y "Dato guardado"
        JLabel check = new JLabel("Guardando...");

        check.setIcon(new ImageIcon(MenuUsuario.class.getResource("/imagenes/spinner.gif")));

        Color customColor = new Color(124, 179, 66);
        check.setForeground(customColor);

        check.setFont(new Font("Roboto", Font.PLAIN, 16));
        check.setBounds(560, 560, 200, 30);
    ...
    private void ocultarCheckVerde() {
        // Remover el "check verde" después de 3 segundos
        contentPane.remove(check);
        contentPane.revalidate();
        contentPane.repaint();
    }

    private void mostrarPregunta() {
        int opcion = JOptionPane.showConfirmDialog(this, "¿ Desea ingresar un nuevo registro para la Reserva Actual ?",
                "Continuar", JOptionPane.YES_NO_OPTION);

        if (opcion == JOptionPane.YES_OPTION) {
            // REINICIAR EL FORMULARIO PARA INGRESAR MÁS REGISTROS
            txtNombre.setText("");
            txtApellido.setText("");
            txtFechaNacimiento.setDate(null);
            txtNacionalidad.setSelectedIndex(0);
            txtTelefono.setText("");
            // PUEDES ACTUALIZAR EL NÚMERO DE RESERVA SI ES NECESARIO.
        } else {
            // MOSTRAR Exito.java SI NO DESEAN INGRESAR MÁS REGISTROS
