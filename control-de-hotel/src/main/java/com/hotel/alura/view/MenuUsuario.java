...
import com.hotel.alura.gestionSesion.GestionDeSesiones;
	...
	public MenuUsuario() {
		...
		btnRegistro.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if (GestionDeSesiones.estaLogueado()) {
					// EL USUARIO ESTA LOGUEADO, ABRE LA VENTANA DE RESERVA.
	                ReservasView reservas = new ReservasView();
	                reservas.setVisible(true);
	                dispose();
				} else {
					// EL USUARIO NO ESTA LOGUEADO, MUESTRA UN MENSAJE DE ERROR.
	                JOptionPane.showMessageDialog(null, "Debes iniciar sesión primero.", "Error", JOptionPane.ERROR_MESSAGE);
				}
			}
		});
		...
		btnBusqueda.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				if (GestionDeSesiones.estaLogueado()) {
					// EL USUARIO ESTA LOGUEADO, ABRE LA VENTANA DE BUSQUEDA.
		...
		btnCerrarSesion.addMouseListener(new MouseAdapter() {
			...
			@Override
			public void mouseClicked(MouseEvent e) {
				GestionDeSesiones.cerrarSesion();
				dispose();
			}
		});
		btnCerrarSesion.setBounds(0, 553, 257, 56);
		btnCerrarSesion.setBackground(new Color(12, 138, 199));
		panelMenu.add(btnCerrarSesion);
		btnCerrarSesion.setLayout(null);
		
		JLabel lblCerrarSesion = new JLabel("Cerrar Sesión");
		lblCerrarSesion.setIcon(new ImageIcon(MenuUsuario.class.getResource("/imagenes/cerrar_sesion.png")));
	    ...
	    //CON ESTE CODIGO MOSTRAREMOS EL NOMBRE DEL USUARIO ACTUAL EN MenuUsuario.
	    //OBTEN EL NOMBRE DEL USUARIO ACTUAL DESDE GestionDeSesiones.
	    String usuarioActual = GestionDeSesiones.obtenerUsuarioActual();
	    // SI NO HAY USUARIO LOGUEADO, PUEDES MOSTRAR UN MENSAJE PREDETERMINADO O REALIZAR OTRA ACCIÓN.
	    String textoBienvenida = (usuarioActual != null) ? usuarioActual : "Invitado";
	    JLabel lblUsuarioActual = new JLabel(textoBienvenida);
	    lblUsuarioActual.setFont(new Font("Roboto", Font.BOLD, 24));
	    lblUsuarioActual.setBounds(455, 234, 147, 46);
